...
# spots seeds
  ...
  Spot.all.each do |spot|
    forecast = Forecast.new(
      ...
      spot_id: spot.id,
      timestamp: DateTime.new(2021,Date.today.month,Date.today.day,9,5,6)
      )
    forecast.save!
  end
  ...
  Spot.all.each do |spot|
    forecast = Forecast.new(
      ...
      high_tide: DateTime.now + 6.25.hours,
      spot_id: spot.id,
      timestamp: DateTime.new(2021,Date.today.month,Date.today.day,16,2,2)
      )
    forecast.save!
  end
  ...
  10.times do |i|
    User.all.each do |user|
      session = Session.new(
        ...
        rating: rand(1..5),
        timestamp: DateTime.now - rand(1..100).day,
        )
      session.save!
    end
  end
